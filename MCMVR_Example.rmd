...
source("~/Dropbox/GitHub/MCMVRDev/R/MCMVR.R")
sourceCpp("~/Dropbox/GitHub/MCMVRDev/src/MCnegloglikCpp.cpp")
```

In this document, we provide a short tutorial on how to use the \texttt{MCMVR} R package. First, we generate data from the ``errors-in-variables'' data generating model described in Section 3.1 of the article. 
...
```
We fit the model using the cross-validation function. There are a key arguments. The first is \texttt{tau.vec}, where one must specify a vector of candidate tuning parameters $\tau$ over which to fit the model: 
...
par(mfrow=c(1,2))
image(betas!=0, col=grey(100:0/100), main=expression(paste("Nonzero entries of ", tilde(beta), "\n")))
image(beta!=0, col=grey(100:0/100), main=expression(paste("Nonzero entries of ", beta["*"], "\n")))
```


We now also fit the model for the nuclear norm penalized version. Note that it is sometimes useful to relax the convergence tolerance for this version. We also turn off the \texttt{quiet} option. Note that \texttt{inner.quiet} should only be used for determining an appropriate value of \texttt{tol}. 
```{r generate RR data}
# ---------------------------------------------------
# Perform 5-fold CV for a grid of tuning parameters 
# ---------------------------------------------------
